...
function isCreditCardNumber(ccn) {
  ...
  const numberWithoutEnd = ccnStr.split('').splice(0, ccnStr.length - 1);
  let sum = 0; let evenIndex = 0;
  for (let i = numberWithoutEnd.length - 1; i >= 0; i -= 1) {
    let element = 0;
    if (evenIndex % 2 === 0) element = Number(numberWithoutEnd[i]) * 2;
    else element = Number(numberWithoutEnd[i]);

    if (element > 9) element -= 9;
    sum += element;
    evenIndex += 1;
  }
  return ccn === Number(`${numberWithoutEnd.join('')}${(10 - (sum % 10)) === 10 ? 0 : 10 - (sum % 10)}`);
}
...
function getDigitalRoot(num) {
  let res = num;
  do {
    res = String(res).split('').reduce((item, sum) => Number(item) + Number(sum));
  } while (res > 9);
  return res;
}
...
function isBracketsBalanced(str) {
  ...
  arr.map((item) => {
    if (brakets[item]) stack.push(item);
    else {
      const lastOne = stack.pop();
      if (!lastOne || brakets[lastOne] !== item) flag = false;
    }
    return item;
  });
...
function getCommonDirectoryPath(pathes) {
  ...
  const minLength = pathes.sort((a, b) => {
  ...
  while (flag) {
    matchedIndex += 1;
    const subStr = pathes[0].substring(0, matchedIndex);
    for (let i = 1; i < pathes.length; i += 1) {
      if (subStr !== pathes[i].substring(0, matchedIndex)) {
        flag = false;
        matchedIndex -= 1;
        break;
      }
    }
    if (matchedIndex >= minLength) flag = false;
  }

  const matchedString = pathes[0].substring(0, matchedIndex);
  return matchedString.substring(0, matchedString.lastIndexOf('/') + 1);
}
