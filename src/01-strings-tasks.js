...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i += 1) {
    if (i === 0) res += '┌';
    else if (i === height - 1) res += '└';
    else res += '│';

    for (let j = 1; j < width - 1; j += 1) {
      if (i === 0 || i === height - 1) res += '─';
      else res += ' ';
    }
...
function encodeToRot13(str) {
  return str.split('').map((item) => {
    let ch = item;
    if ('abcdefghijklmnopqrstuvwxyz'.includes(item)) ch = String.fromCharCode('a'.charCodeAt(0) + ((item.charCodeAt(0) - 'a'.charCodeAt(0) + 13) % 26));
    if ('ABCDEFGHIJKLMNOPQRSTUVWXYZ'.includes(item)) ch = String.fromCharCode('A'.charCodeAt(0) + ((item.charCodeAt(0) - 'A'.charCodeAt(0) + 13) % 26));
    return ch;
  })
    .join('');
